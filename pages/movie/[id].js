...
//           </h1>
//           <p className={styled.plot}>{movie.Plot}</p>
//           <p className={styled.director}><strong>Director: </strong>{movie.Director}</p>
//           <p className={styled.actors}><strong>Actors: </strong>{movie.Actors}</p>
//           <p className={styled.genre}><strong>Genre: </strong>{movie.Genre}</p>
...
//   const fetchMovieDetails = async () => {
//     const response = await axios.get(`httpі://www.omdbapi.com/?apikey=fa586b6&i=${id}`);
//     setMovie(response.data);
//   }
...
const MovieDetails = ({ id }) => {
  ...
  const fetchMovieDetails = async () => {
    const response = await axios.get(`http://www.omdbapi.com/?apikey=fa586b6&i=${id}`);
    setMovie(response.data);
  }
  ...
  return (
    <div className={style.header}>
      <Header />
      <Link href="/" className={styled.back_btn}>  Back to Home</Link>
      <div className={styled.page_info}>
        {movie.Poster !== "N/A" ? (
          <img
            src={movie.Poster}
            alt={movie.Title}
            className={styled.page_img}
          />
        ) : (
          <div>No image available</div>
        )}
        <div>
          ...
          <p className={styled.plot}>{movie.Plot}</p>
          <p className={styled.director}>
            <strong>Director: </strong>
            {movie.Director}
          </p>
          <p className={styled.actors}>
            <strong>Actors: </strong>
            {movie.Actors}
          </p>
          <p className={styled.genre}>
            <strong>Genre: </strong>
            {movie.Genre}
          </p>
          <p className={styled.runtime}>
